...
export function humanOffset(label) {
  ...
  // toHuman() из Luxon не склоняет по-русски, а setLocale у Duration нет
  // (он только у DateTime) — считаем сами.
  const MIN = 60_000, HOUR = 3_600_000, DAY = 86_400_000, WEEK = 7 * DAY;
  let n, forms;
  if (ms % WEEK === 0) [n, forms] = [ms / WEEK, ['неделю', 'недели', 'недель']];
  else if (ms % DAY === 0) [n, forms] = [ms / DAY, ['день', 'дня', 'дней']];
  else if (ms % HOUR === 0) [n, forms] = [ms / HOUR, ['час', 'часа', 'часов']];
  else [n, forms] = [Math.round(ms / MIN), ['минуту', 'минуты', 'минут']];

  return `за ${n} ${pluralRu(n, forms)}`;
}
