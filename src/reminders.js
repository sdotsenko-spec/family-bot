...
// Дублировать ли в общий чат короткий пинг «@исполнитель, глянь личку».
//   all — на каждое напоминание (по умолчанию)
...
function buildKeyboard(task) {
  ...
  if (task.recurrence_id) {
    kb.row().text('🚫 Отключить повтор', `drop_recur:${task.recurrence_id}`);
  }
  return kb;
}
 ...
 */
/** Общий семейный чат: сначала переменная окружения, потом то, что запомнил /start в группе. */
async function familyChat() {
  return process.env.FAMILY_CHAT_ID || (await getSetting('family_chat_id')) || null;
}

/** Первое ли это напоминание по задаче (никакое другое ещё не отправлялось). */
async function isFirstReminder(reminder) {
  const { rows } = await q(
    `select 1 from reminders where task_id = $1 and status = 'sent' and id <> $2 limit 1`,
    [reminder.task_id, reminder.id]
  );
  return rows.length === 0;
}

/**
 * Короткий пинг в общую группу. Намеренно без названия задачи и без времени —
 * иначе смысл доставки в личку теряется: содержимое всё равно видит вся группа.
 */
function renderPing(task, assignee) {
  return `📨 ${mention(assignee)}, проверь личку`;
}

/**
 * Куда летит одно напоминание. Чистая функция — тестируется без БД и Telegram.
 * Первый элемент списка — основное сообщение (с кнопками), его message_id сохраняем.
 */
export function chooseTargets({ reminder, task, assignee, familyChatId, familyThread, isFirst }) {
  const isEscalation = reminder.label === 'escalation';
  const dmChatId = assignee ? task.dm_chat_id : null;
  ...
  // Личка исполнителя недоступна (не писал боту /start) — как раньше, в чат задачи
  if (!dmChatId) {
    const hint = assignee
      ? '\n\n<i>Не могу писать в личку — напишите мне /start в личных сообщениях.</i>'
      : '';
    return [
      {
        chat: task.chat_id || groupChatId,
        thread: task.thread_id || undefined,
        text: body + hint,
        keyboard: true,
      },
    ];
  ...
  const targets = [{ chat: dmChatId, text: body, keyboard: true }];

  const pingNow =
    GROUP_PING === 'all' || (GROUP_PING === 'key' && (reminder.label === 'due' || isFirst));

  // Задача создана в этой же личке — дублировать некуда и незачем
  if (pingNow && groupChatId && String(groupChatId) !== String(dmChatId)) {
    targets.push({
      chat: groupChatId,
      thread: groupThread,
      text: renderPing(task, assignee),
      keyboard: false,
    });
  }
...
export async function dispatchDueReminders(bot) {
  ...
  const familyChatRaw = await familyChat();
  const familyThread = await getSetting('family_thread_id');
  let sent = 0;
  ...
  for (const reminder of due) {
    try {
      ...
      const targets = chooseTargets({
        ...
        task,
        assignee,
        familyChatId: familyChatRaw,
        familyThread,
        isFirst: GROUP_PING === 'key' ? await isFirstReminder(reminder) : false,
      });
      ...
      for (const [i, target] of targets.entries()) {
        try {
          const msg = await bot.api.sendMessage(target.chat, target.text, {
            parse_mode: 'HTML',
            message_thread_id: target.thread || undefined,
            reply_markup: target.keyboard ? buildKeyboard(task) : undefined,
            link_preview_options: { is_disabled: true },
          });
          if (i === 0) primaryMessageId = msg.message_id;
        } catch (e) {
          // Падение пинга в группу не должно ронять основную доставку
          if (i === 0) throw e;
          console.warn(`[reminders] пинг в группу не ушёл (#${reminder.id}):`, e.message);
        }
