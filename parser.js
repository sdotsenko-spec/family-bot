...
export function parseFallback(input, tz = TZ, now = DateTime.now().setZone(tz)) {
  ...
  if (remind) {
    const re = new RegExp(`за\\s+(\\d+)?\\s*(сутки|суток|день|дня|дней|час[а-яё]*|минут[а-яё]*|недел[а-яё]*)`, 'giu');
    let r;
    while ((r = re.exec(remind[1]))) {
      const n = r[1] ? Number(r[1]) : 1;
      const u = r[2].toLowerCase();
      if (u.startsWith('сут') || u.startsWith('де') || u.startsWith('дн')) offsets.push(`${n * 24}h`);
      else if (u.startsWith('час')) offsets.push(`${n}h`);
      else if (u.startsWith('минут')) offsets.push(`${n}m`);
      else if (u.startsWith('недел')) offsets.push(`${n * 7}d`);
    }
    if (offsets.length) {
      text = (text.slice(0, remind.index) + ' ' + text.slice(remind.index + remind[0].length))
        .replace(/\s+/g, ' ');
    }
  }
  ...
  if ((m = eat(`${B}через\\s+(\\d+)\\s*(минут[а-яё]*|час[а-яё]*|дн[а-яё]*|день|недел[а-яё]*)${E}`))) {
    const n = Number(m[1]);
  ...
  const title = text
    .replace(/\s+/g, ' ')
    .replace(new RegExp(`^(?:напомни|поставь|задача|таска|добавь|нужно|надо)${E}\\s*`, 'iu'), '')
